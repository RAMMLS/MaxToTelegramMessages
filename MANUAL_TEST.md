...
## 8. Ежедневный heartbeat

1. Включить `DAILY_REPORT_ENABLED=true` и запустить мост с чистым report state.
2. Получить первый служебный отчёт в целевом Telegram-чате.
3. Перезапустить мост до истечения 24 часов.

- [ ] Первый отчёт не содержит текста, имён, названий или MAX/Telegram ID.
- [ ] При активной сессии отчёт показывает `MAX: подключён`.
- [ ] Счётчики выбранных/доставленных сообщений совпадают с тестовыми событиями.
- [ ] Pending/failed outbox отражены отдельными числами.
- [ ] Рестарт не отправляет второй отчёт до истечения сохранённого интервала.
- [ ] При разрыве MAX отчёт жёлтый и показывает `переподключается`.
- [ ] Ошибка heartbeat Telegram не останавливает пересылку MAX-сообщений.
