...
`MAX_DEVICE_ID` необязателен. Если он не задан, UUID создаётся при старте. После
авторизованной проверки стоит подтвердить, требуется ли сохранять один device ID
между рестартами.

## Надёжность и локальное состояние

Перед отправкой нормализованное выбранное сообщение записывается в
`BRIDGE_STATE_DB`. После успешного Telegram response запись помечается
...
- падение между MAX ack и записью в outbox создаёт небольшое окно потери;
